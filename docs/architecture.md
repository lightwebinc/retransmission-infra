...
The `memory` + `REDIS_ADDR` mode (row 2) suits small multi-endpoint
deployments: each endpoint maintains its own frame cache (preserving
MISS-escalation logic), while Redis deduplicates redundant retransmits when
several endpoints receive NACKs for the same gap simultaneously.
